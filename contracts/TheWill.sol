...
contract devWill {
    ...
    // address type definition options
    //- address myAddress;
    //- address payable myAddress;
    //- address payable[] myAddresses; // array of addreses all of them payable
     ...
     */
    modifier onlyOwner() {
        // the require() used to initiate required condition that must happen before moving on, a bit like if()
        // there qare several "gates" methods: if(msg.sender != owner) { one of the handlers below  }

        // 1) throw; // is the older way to handle exception >0.4, deprected
        // 2) revert('info msg') // undo all state changes, allow you to return a value, will refund any remaining gas to the caller
        // 3) require(msg.sender == owner, 'info message'); // not stopping the script, just used as gate, as a polite managerial type, who calls out your errors, but is more forgiving
        // 4) assert(msg.sender == owner); // more strict, treated as exception that should stop the script. as an overly assertive bully, who steals all your gas
